    ...
    */
    public static class SymmetricCryptoProvider
    {
        ...
        public static byte[] SimpleEncryptWithPassword(byte[] secretMessage, string password)
        {
            ...
            byte[] cryptKey;
            using (var generator = new Rfc2898DeriveBytes(password, SaltBitSize / 8, Iterations, HashAlgorithmName.SHA256))
            {
        ...
        public static byte[] SimpleDecryptWithPassword(byte[] encryptedMessage, string password)
        {
            ...
            //Generate crypt key
            using (var generator = new Rfc2898DeriveBytes(password, cryptSalt, Iterations, HashAlgorithmName.SHA256))
            {
                cryptKey = generator.GetBytes(KeyBitSize / 8);
            }
